...
export default function SupplierCategories() {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      await Promise.all([loadAllAvailableCategories(), loadDeactivatedCategories(), fetchCategoryMargins()]);
    } finally {
      setLoading(false);
    }
  }
  ...
  const fetchCategoryMargins = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/product-margin/get-category-margin/supplier`
      );
      if (response.ok) {
  ...
  async function loadDeactivatedCategories() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/categories/list-activated-supplier-category`,
        { headers: { 'Content-Type': 'application/json' } }
      );
  ...
  const handleAddMargin = async (category) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/product-margin/add-category-margin/supplier`,
        {
          method: "POST",
          headers: {
  ...
  const handleDeleteMargin = async (category) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/product-margin/delete-category-margin/supplier?categoryId=${category.groupId}&supplierId=${supplier.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const isCategoryDeactivated = (categoryId) => {
    const isDeactivated = deactivatedCategories.some(deactivated => 
  ...
  const categoriesWithStatus = useMemo(() => {
    ...
    return allAvailableCategories.map(category => {
  ...
  return (
    <div className="px-6 py-4">
      ...
      <div className="mb-4 text-sm text-gray-600">
        ...
        <p>Deactivated Categories: {deactivatedCategories.filter(d => String(d.supplierId) === String(supplier.id)).length}</p>
      ...
      <Table>
        ...
        <TableBody>
          ...
          {!loading && categoriesWithStatus.map((cat) => {
            ...
            <TableRow>
              <TableCell colSpan={5} className="text-center py-8 text-gray-500">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm">Try fetching data or check if the supplier has any products</p>
                </div>
              </TableCell>
            </TableRow>
          )}
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div>
              <span className="text-green-600 font-medium">
                Active Categories: {categoriesWithStatus.filter(c => c.isActive).length}
              </span>
            </div>
            <div>
              <span className="text-red-600 font-medium">
                Deactivated Categories: {categoriesWithStatus.filter(c => !c.isActive).length}
              </span>
            </div>
            <div>
              <span className="text-blue-600 font-medium">
                Categories with Margins: {Object.keys(categoryMargins).filter(key => key.startsWith(`${supplier.id}-`)).length}
              </span>
            </div>
          </div>
        </div>
