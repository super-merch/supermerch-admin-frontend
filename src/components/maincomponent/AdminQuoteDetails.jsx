...
import { ArrowLeft, Calendar, User, Package, MapPin, FileText, MessageSquare, Edit } from 'lucide-react';
...
export default function AdminQuoteDetails() {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-4 py-2 rounded-full text-sm font-bold border-2 ${colors[status]}`}>
        {status.toUpperCase()}
      </span>
    );
  };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-4 gap-4 pt-4 border-t">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{quote.items.length}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Subtotal</p>
              <p className="text-2xl font-bold text-gray-900">${quote.subtotal.toFixed(2)}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-green-600">${quote.total.toFixed(2)}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              ...
              {quote.customer.defaultAddress && Object.keys(quote.customer.defaultAddress).length > 0 && (
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    {quote.customer.defaultAddress.firstName && (
                      <p className="font-medium text-gray-900">
                        {quote.customer.defaultAddress.firstName} {quote.customer.defaultAddress.lastName}
                      </p>
                    )}
                    {quote.customer.defaultAddress.companyName && (
                      <p className="text-gray-700">{quote.customer.defaultAddress.companyName}</p>
                    )}
                    {quote.customer.defaultAddress.addressLine && (
                      <p className="text-gray-700">{quote.customer.defaultAddress.addressLine}</p>
                    )}
                    {quote.customer.defaultAddress.additional && (
                      <p className="text-gray-600 text-sm">{quote.customer.defaultAddress.additional}</p>
                    )}
                    ...
                    {quote.customer.defaultAddress.country && (
                      <p className="text-gray-700">{quote.customer.defaultAddress.country}</p>
                    )}
                  </div>
                </div>
              )}

              {quote.customer.defaultShippingAddress && Object.keys(quote.customer.defaultShippingAddress).length > 0 && (
                <div className="pt-3 border-t">
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-2 flex items-center gap-1">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    {quote.customer.defaultShippingAddress.firstName && (
                      <p className="font-medium text-gray-900">
                        {quote.customer.defaultShippingAddress.firstName} {quote.customer.defaultShippingAddress.lastName}
                      </p>
                    )}
                    {quote.customer.defaultShippingAddress.companyName && (
                      <p className="text-gray-700">{quote.customer.defaultShippingAddress.companyName}</p>
                    )}
                    {quote.customer.defaultShippingAddress.addressLine && (
                      <p className="text-gray-700">{quote.customer.defaultShippingAddress.addressLine}</p>
                    )}
                    <p className="text-gray-700">
                      {[
                    ...
                    {quote.customer.defaultShippingAddress.country && (
                      <p className="text-gray-700">{quote.customer.defaultShippingAddress.country}</p>
                    )}
                    {(quote.customer.defaultShippingAddress.email || quote.customer.defaultShippingAddress.phone) && (
                      <div className="mt-2 pt-2 border-t border-gray-200 text-sm">
                        {quote.customer.defaultShippingAddress.email && (
                          <p className="text-gray-600">📧 {quote.customer.defaultShippingAddress.email}</p>
                        )}
                        {quote.customer.defaultShippingAddress.phone && (
                          <p className="text-gray-600">📞 {quote.customer.defaultShippingAddress.phone}</p>
                        )}
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {quote.items.map((item, index) => (
                <div key={item._id || index} className="border-2 border-gray-200 rounded-lg p-5 hover:border-blue-300 transition">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-2xl font-bold text-blue-600">${item.subtotal.toFixed(2)}</p>
                        </div>
                      </div>
                      ...
                      {item.decoration && item.decoration.method && (
                        <div className="border bg-gray-100 rounded-lg p-3 mb-3">
                          <p className="text-xs font-semibold text-blue-800 uppercase mb-2">Decoration Details</p>
                          <div className="grid grid-cols-3 gap-3 text-sm">
                            <div>
                              <p className="text-gray-600">Method</p>
                              <p className="font-semibold text-gray-900">{item.decoration.method}</p>
                            </div>
                            <div>
                              <p className="text-gray-600">Price</p>
                              <p className="font-semibold text-gray-900">${item.decoration.price.toFixed(2)}</p>
                            </div>
                            {item.decoration.description && (
                              <div>
                                <p className="text-gray-600">Description</p>
                                <p className="font-semibold text-gray-900">{item.decoration.description}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {item.customDescription && (
                        <div className="border bg-gray-100 rounded p-3 mb-2">
                          <p className="text-xs text-blue-800 font-semibold uppercase mb-1">Custom Description</p>
                          <p className="text-sm text-gray-900">{item.customDescription}</p>
                        </div>
                      )}
              ...
              <div className="space-y-3">
                {quote.comments.map((comment, index) => (
                  <div key={index} className="border-l-4 border-blue-500 bg-blue-50 p-4 rounded-r-lg">
                    <p className="text-gray-900 font-medium mb-2">{comment.text}</p>
                    <div className="flex items-center gap-3 text-sm text-gray-600">
                      <span className="font-semibold">👤 {comment.addedBy}</span>
                      <span>•</span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-2">
              <p className="text-lg font-semibold text-gray-900">{quote.createdBy.name}</p>
              ...
              {quote.sentAt && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{formatDate(quote.sentAt)}</p>
                  </div>
                </div>
              )}
              {quote.viewedAt && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{formatDate(quote.viewedAt)}</p>
                  </div>
                </div>
              )}
              {quote.deadline && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{formatDate(quote.deadline)}</p>
                  </div>
                </div>
              )}
              {quote.respondedAt && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{formatDate(quote.respondedAt)}</p>
                  </div>
                </div>
              )}
              {quote.expiresAt && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{formatDate(quote.expiresAt)}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {quote.reference && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <p className="text-gray-900 font-semibold text-lg">{quote.reference}</p>
            </div>
          )}
          ...
          {quote.companyDetails && quote.companyDetails.name && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-2">
                ...
                <p className="font-bold text-gray-900 text-lg">{quote.companyDetails.name}</p>
                {quote.companyDetails.address && (
                  <p className="text-sm text-gray-600">{quote.companyDetails.address}</p>
                )}
                {quote.companyDetails.phone && (
                  <p className="text-sm text-gray-600">📞 {quote.companyDetails.phone}</p>
                )}
                {quote.companyDetails.email && (
                  <p className="text-sm text-gray-600">📧 {quote.companyDetails.email}</p>
                )}
              </div>
            </div>
          )}
