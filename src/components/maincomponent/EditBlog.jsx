...
const EditBlog = () => {
  ...
  const [existingAdditionalImages, setExistingAdditionalImages] = useState([]); // Track existing images
  ...
  const fetchBlogData = async () => {
    ...
    try {
      ...
      setExistingAdditionalImages(blog.additionalImages || []);
  ...
  const handleSubmit = async () => {
    ...
    formData.append("keepExistingImages", JSON.stringify(existingAdditionalImages));
    ...
    try {
        let response;
        if (isEditMode) {
            response = await axios.put(`${backednUrl}/api/blogs/update-blog/${id}`, formData, {
                headers: { "Content-Type": "multipart/form-data" },
                timeout: 120000
            });
            toast.success("Blog updated successfully");
        } else {
            response = await axios.post(`${backednUrl}/api/blogs/save-blog`, formData, {
                headers: { "Content-Type": "multipart/form-data" },
                timeout: 120000
            });
        ...
        if (error.code === 'ECONNABORTED') {
            toast.error("Upload timeout. Please try with smaller images or check your internet connection.");
        } else {
  ...
  // Update removeAdditionalImage function
  const removeAdditionalImage = (index) => {
    const imageToRemove = additionalImages[index];

    // Remove from additionalImages (display array)
    setAdditionalImages((prev) => prev.filter((_, i) => i !== index));

    // If it's an existing image (string URL), remove from existingAdditionalImages
    if (typeof imageToRemove === "string") {
      setExistingAdditionalImages((prev) =>
        prev.filter((img) => img !== imageToRemove)
      );
    } else {
      // If it's a new file, remove from newAdditionalImages
      setNewAdditionalImages((prev) =>
        prev.filter((file) => file !== imageToRemove)
      );
    }
  };

  const handleCancel = () => {
    if (title || value || selectedImages || newAdditionalImages.length > 0 || 
        JSON.stringify(existingAdditionalImages) !== JSON.stringify(additionalImages.filter(img => typeof img === 'string'))) {
      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-sm border">
        ...
        <div className="flex flex-col gap-2 mb-6 text-gray-500">
          ...
          <p className="text-xs text-gray-400 mb-2">
            These images will be displayed within the blog content
          </p>
