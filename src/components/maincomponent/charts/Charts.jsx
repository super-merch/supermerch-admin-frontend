...
const Charts = ({ isOpen }) => {
    ...
    const { users, orders, orderCompleted, orderPending, products, blogs, quoteData } = useContext(AdminContext);
    return (
        <div className={`${isOpen ? 'max-w-[95%]' : 'max-w-full'} lg:px-3 md:px-3 px-0 w-full mx-auto my-5 max-sm:p-2`}>
            <div className='flex flex-wrap gap-3'>

                <div className='flex flex-col justify-center items-center gap-2 bg-white p-6 min-w-52 rounded border-2 shadow-sm 
                ...
                <div className='flex flex-col justify-center items-center gap-2 bg-white p-6 min-w-52 rounded border-2 shadow-sm border-gray-100 cursor-pointer hover:scale-105 transition-all max-sm:w-full'>
                    ...
                    <div className="text-center">   
                        ...
                        <p className='text-xl font-semibold text-gray-600'>{quoteData?.length}</p>
                    </div>
                </div>
