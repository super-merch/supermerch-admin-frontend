...
const Trending = () => {
    ...
    const handleAddProduct = async (productId) => {
        setIsLoading(true);
        try {
            const response = await addTrending({ productId });
            if (response.data.success) {
                toast.success(
                    response.data.message || "Product added to Trending"
                );
                setAddModal(false);
                setSearchTerm("");
                setSearchResults([]);
            ...
            toast.error(
                error.response?.data?.message || "Error adding product to Trending"
            );
        } finally {
            setIsLoading(false);
        }
    ...
    const handleDelete = async (e) => {
        ...
        try {
            const response = await removeTrending(remove_id);
            if (response.data.success) {
                toast.success(
                    response.data.message || "Product removed from Trending"
                );
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error removing product from Trending"
            );
        } finally {
    ...
    const fetchAllForExport = async () => {
        try {
            const response = await getTrending({ page: 1, limit: 10000 });
    ...
    document.title = `Trending Products | ${adminData.companyName}`;
