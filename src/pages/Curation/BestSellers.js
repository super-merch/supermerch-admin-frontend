...
const BestSellers = () => {
    ...
    const handleAddProduct = async (productId) => {
        setIsLoading(true);
        try {
            const response = await addBestSeller({ productId });
            if (response.data.success) {
                toast.success(
                    response.data.message || "Product added to Best Sellers"
                );
                setAddModal(false);
                setSearchTerm("");
                setSearchResults([]);
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error adding product to Best Sellers"
            );
        } finally {
            setIsLoading(false);
        }
    ...
    const handleDelete = async (e) => {
        ...
        try {
            const response = await removeBestSeller(remove_id);
            if (response.data.success) {
                toast.success(
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error removing product from Best Sellers"
            );
        } finally {
    ...
    const fetchAllForExport = async () => {
        try {
            const response = await getBestSellers({ page: 1, limit: 10000 });
    ...
    document.title = `Best Sellers | ${adminData.companyName}`;
