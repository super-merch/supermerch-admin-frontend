...
const NewArrivals = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const response = await getNewArrivals(params);
    ...
    const handleAddProduct = async (productId) => {
        setIsLoading(true);
        try {
            const response = await addNewArrival({ productId });
            if (response.data.success) {
                toast.success(
                    response.data.message || "Product added to New Arrivals"
                );
                setAddModal(false);
                setSearchTerm("");
                setSearchResults([]);
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error adding product to New Arrivals"
            );
        } finally {
            setIsLoading(false);
        }
    ...
    const handleDelete = async (e) => {
        ...
        try {
            const response = await removeNewArrival(remove_id);
            if (response.data.success) {
                toast.success(
                    response.data.message || "Product removed from New Arrivals"
                );
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error removing product from New Arrivals"
            );
        } finally {
