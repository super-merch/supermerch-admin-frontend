...
const HourProduction = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const response = await get24Hour(params);
            ...
            toast.error("Failed to fetch 24-hour production products");
            setData([]);
            setTotalRows(0);
        } finally {
            setLoading(false);
    ...
    const handleAddProduct = async (productId) => {
        setIsLoading(true);
        try {
            const response = await add24Hour({ productId });
            if (response.data.success) {
                toast.success(
                    response.data.message ||
                        "Product added to 24-Hour Production"
                );
                setAddModal(false);
                setSearchTerm("");
                setSearchResults([]);
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error adding product to 24-Hour Production"
            );
        } finally {
            setIsLoading(false);
        }
    ...
    const handleDelete = async (e) => {
        ...
        try {
            const response = await remove24Hour(remove_id);
            if (response.data.success) {
                toast.success(
                    response.data.message ||
                        "Product removed from 24-Hour Production"
                );
            ...
            toast.error(
                error.response?.data?.message ||
                    "Error removing product from 24-Hour Production"
            );
        } finally {
        ...
        <React.Fragment>
            ...
            <Modal isOpen={addModal} toggle={toggleAddModal} centered size="lg">
                <ModalHeader toggle={toggleAddModal}>
                    Add Product to 24-Hour Production
                </ModalHeader>
                <ModalBody>
