...
} from "../../functions/CMS/homePageBannerFunc";
...
const HomePageBanner = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <img
          src={`${api.API_URL}/uploads/homePageBanner/${row.imageUrl}`}
          alt="banner"
  ...
  const fetchHomePageBanners = useCallback(async () => {
    ...
    await axios
      .get(`/api/home-page-banners?${params}`, {
      ...
      createHomePageBanner(formData)
        .then((res) => {
          if (res.data.success) {
            ...
            setFormErrors({});
            fetchHomePageBanners();
            toast.success("Home Page Banner Added Successfully");
          } else {
            toast.error(res.data.message || "Failed to create Home Page Banner");
          }
        })
        .catch((err) => {
          console.log(err);
          toast.error(err.response?.data?.message || "Error creating Home Page Banner");
        })
  ...
  const handleUpdate = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      updateHomePageBanner(_id, formData)
        .then((res) => {
          if (res.data.success) {
            ...
            toast.error(res.data.message || "Failed to update Home Page Banner");
          }
        })
        .catch((err) => {
          console.log(err);
          toast.error(err.response?.data?.message || "Error updating Home Page Banner");
        })
        .finally(() => setIsLoading(false));
    }
  };
  ...
  const handleDelete = (e) => {
    ...
    deleteHomePageBanner(remove_id)
      .then((res) => {
        if (res.data.success) {
          setmodal_delete(!modal_delete);
          fetchHomePageBanners();
          toast.success("Home Page Banner Deleted Successfully");
        } else {
          toast.error(res.data.message || "Cannot delete Home Page Banner");
        }
      })
      .catch((err) => {
        console.log(err);
        toast.error("Cannot delete Home Page Banner");
      })
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          if (bannerData.imageUrl) {
            ...
            setImagePreview(`/uploads/homePageBanner/${bannerData.imageUrl}`);
          }
        }
      })
      .catch((err) => {
