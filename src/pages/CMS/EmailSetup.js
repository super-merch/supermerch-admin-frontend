...
import { createEmailSetup, deleteEmailSetup, getEmailSetupById, updateEmailSetup } from "../../functions/CMS/emailSetupFunc";
...
const EmailSetup = () => {
  ...
  const handleTog_edit = (_id) => {
    ...
    getEmailSetupById(_id)
      ...
      .catch((err) => {
        console.log(err);
        toast.error("Error fetching email setup data!");
      }).finally(() => {
        setLoading(false);
      });
  };
      ...
      createEmailSetup(values)
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.data && error.response.data.message) {
            ...
            toast.error("Failed to create email setup!");
          }
        }).finally(() => {
          setLoading(false);
        });
  ...
  const handleDelete = async(e) => {
    ...
    try {
        ...
        if(res.data.success) {
            ...
            setDeleteErrorMessage(res.data.message || "Failed to delete email setup");
  ...
  const validate = (values) => {
    ...
    } else if (isNaN(values.port) || values.port < 1 || values.port > 65535) {
  ...
  const fetchCountries = useCallback(async () => {
    ...
    try {
      ...
      setCountries([]);
      setTotalRows(0);
      toast.error("Failed to fetch email setups!");
    } finally {
      setLoading(false);
    }
